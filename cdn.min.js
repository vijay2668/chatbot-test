...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  const handleFaqSubmit = async (query) => {
    if (count === 0) {
      const saveSession = await fetch(
        "https://web-production-949d.up.railway.app/api/save-session",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json"
      ...
      const saveSessionResult = await saveSession.json();
      messages_id = saveSessionResult;
    }

    console.log(messages_id);
    count++;

    if (!messages_id) return;
