...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  userInputField.addEventListener("input", function () {
    ...
    sendButton.style.display = userInputField.value.trim() !== "" ? "block" : "none";
  });
  ...
  const handleFaqSubmit = async (query) => {
    ...
    messages_list.innerHTML += `<div id="remove" class="relative p-4 flex space-x-1 items-start justify-start">
    ...
    const get_bot_message = await fetch(
      "https://web-production-949d.up.railway.app/api/get-bot-message",
      {
        ...
        body: JSON.stringify({
          ...
          chatbot_id: chatbotUI?._id,
          query: query,
          length_file_ids: chatbotUI?.length_file_ids,
          profile_id: chatbotUI?.profile?._id,
          count: count
        })
      }
    );
  ...
  faqs.forEach(function (faq) {
    ...
    button.style.background =
      chatbotUI && chatbotUI.accent_colour ? chatbotUI.accent_colour : "";
    ...
    button.className =
      "text-sm cursor-pointer rounded-xl text-white px-2 py-1 w-fit h-fit whitespace-nowrap";
  ...
  form.addEventListener("submit", async function (event) {
    ...
    messages_list.innerHTML += message_template({
      role: bot_message?.role,
      message: bot_message?.follow_up_questions
    });
    scrollToBottom();
  });
});
