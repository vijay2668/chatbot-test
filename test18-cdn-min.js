...
// function getLighterOrDarkerColor(color) {
...
//   // Calculate color brightness using the formula: (R * 299 + G * 587 + B * 114) / 1000
//   let brightness = (r * 299 + g * 587 + b * 114) / 1000;

//   // Determine if the color is darker or lighter based on brightness
//   if (brightness >= 128) {
//     // Color is lighter, make it darker by reducing each RGB component
//     r = Math.floor(r * 0.6); // Adjusted factor for a lighter result
//     g = Math.floor(g * 0.6);
//     b = Math.floor(b * 0.6);
//   } else {
//     // Color is darker, make it lighter by increasing each RGB component
//     r = Math.min(Math.floor(r * 1.5), 255); // Adjusted factor for a darker result
//     g = Math.min(Math.floor(g * 1.5), 255);
//     b = Math.min(Math.floor(b * 1.5), 255);
//   }

//   // Convert back to hexadecimal format
//   let darkerOrLighterColor =
//     "#" + ((1 << 24) + (r << 16) + (g << 8) + b).toString(16).slice(1);

//   return darkerOrLighterColor;
// }
...
<div class="chatui scale-0 absolute sm:right-4 origin-bottom-right bottom-16 py-2 transition-all duration-200  px-4 sm:px-0 w-full h-[80vh] rounded-xl sm:max-w-[400px] z-[99999]">
  <div class="max-w-full w-full bg-transparent justify-end h-full items-end flex flex-col space-y-4 right-0">
    <div class="h-full border bg-white flex flex-col rounded-2xl overflow-hidden w-full">
      ...
      <div id="chat-section" class="hidden flex flex-col overflow-hidden w-full h-full">
        <div class="bg-primary relative rounded-t-2xl flex items-center justify-start w-full h-fit p-2">
          <button onclick="navigate(localStorage.getItem('last_location') || 'home'); let sessions = JSON.parse(localStorage.getItem('visitor_sessions')); sessions && conversation_template(sessions);" class="w-12 h-12 flex items-center justify-center">
            <svg stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 320 512" class="w-5 h-5 text-white" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg"><path d="M9.4 233.4c-12.5 12.5-12.5 32.8 0 45.3l192 192c12.5 12.5 32.8 12.5 45.3 0s12.5-32.8 0-45.3L77.3 256 246.6 86.6c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0l-192 192z"></path></svg>
          </button>
          ...
          <div class="flex items-center space-x-3 p-1 py-3">
            ...
            <div class="flex flex-col tracking-wider">
              ...
              <div class="flex items-center space-x-1 h-fit text-[0.7rem]">
                <svg stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 512 512" class="text-white text-[0.65rem]" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg"><path d="M464 256A208 208 0 1 1 48 256a208 208 0 1 1 416 0zM0 256a256 256 0 1 0 512 0A256 256 0 1 0 0 256zM232 120V256c0 8 4 15.5 10.7 20l96 64c11 7.4 25.9 4.4 33.3-6.7s4.4-25.9-6.7-33.3L280 243.2V120c0-13.3-10.7-24-24-24s-24 10.7-24 24z"></path></svg>
            ...
            <div class="absolute z-10 top-0 left-0 flex flex-col space-y-4 overflow-scroll scrollbar-hide p-4 w-full h-full">
              ...
              <div class="bg-white rounded-xl p-2 shadow-md flex flex-col space-y-4">
                <div>
                  ...
                  class="flex bg-gray-100 w-full text-sm h-10 rounded-full border-input px-3 py-2 ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-ring disabled:cursor-not-allowed disabled:opacity-50 scrollbar-hide focus-visible:ring-0 focus-visible:ring-offset-0 border"
                  />
                </div>
                <ul id="faq-list" class="text-gray-600 divide-y flex flex-col overflow-y-scroll overflow-x-hidden scrollbar-hide h-full w-full">
                </ul>
              </div>
            </div>
          </div>
        ...
        <div id="navigationTabs" class="w-full px-4 py-2 border-t">
          <div class="flex space-x-2 items-center text-gray-400">
            ...
            <button id="homeButton" class="flex flex-col items-center hover:text-[var(--main-color)] text-[var(--main-color)] transition-all w-full">
              <svg stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 576 512" class="w-5 h-5" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg"><path d="M575.8 255.5c0 18-15 32.1-32 32.1h-32l.7 160.2c0 2.7-.2 5.4-.5 8.1V472c0 22.1-17.9 40-40 40H456c-1.1 0-2.2 0-3.3-.1c-1.4 .1-2.8 .1-4.2 .1H416 392c-22.1 0-40-17.9-40-40V448 384c0-17.7-14.3-32-32-32H256c-17.7 0-32 14.3-32 32v64 24c0 22.1-17.9 40-40 40H160 128.1c-1.5 0-3-.1-4.5-.2c-1.2 .1-2.4 .2-3.6 .2H104c-22.1 0-40-17.9-40-40V360c0-.9 0-1.9 .1-2.8V287.6H32c-18 0-32-14-32-32.1c0-9 3-17 10-24L266.4 8c7-7 15-8 22-8s15 2 21 7L564.8 231.5c8 7 12 15 11 24z"></path></svg>
              <div class="text-xs mt-0.5 font-semibold">Home</div>
            </button>
            <!-- chat -->
            ...
            <!--   <i class="far fa-envelope icon-center text-[1.3rem] leading-none"></i>-->
            <!--   <div class="text-xs mt-0.5 font-semibold">Chat</div>-->
            <!-- </button>-->
            <!-- messages -->
            <button id="messagesButton" class="flex flex-col items-center hover:text-[var(--main-color)] transition-all w-full">
              <svg stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 512 512" class="w-5 h-5" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg"><path d="M64 112c-8.8 0-16 7.2-16 16v22.1L220.5 291.7c20.7 17 50.4 17 71.1 0L464 150.1V128c0-8.8-7.2-16-16-16H64zM48 212.2V384c0 8.8 7.2 16 16 16H448c8.8 0 16-7.2 16-16V212.2L322 328.8c-38.4 31.5-93.7 31.5-132 0L48 212.2zM0 128C0 92.7 28.7 64 64 64H448c35.3 0 64 28.7 64 64V384c0 35.3-28.7 64-64 64H64c-35.3 0-64-28.7-64-64V128z"></path></svg>
              <div class="text-xs mt-0.5 font-semibold">Messages</div>
            </button>
          </div>
        </div>


        <div class="bg-gray-50 text-center text-sm py-1 text-gray-500">
          <a href="#">
            Powered by <strong>DocMate</strong>
          </a>
        </div>
...
styleElem.innerHTML = `
  ...
  #widget *:not(.fa-solid, .fas, .far, .fa-regular) {
...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  const fetch_initial = await fetch(
    "https://chatbot-server-2668.vercel.app/api/fetch-chatbot",
    {
      method: "POST",
      headers: {
  ...
  // const buttonColor = getLighterOrDarkerColor(chatbotUI?.accent_colour);
  ...
  faqs.forEach(function (faq) {
    document.getElementById("faq-list").innerHTML += `
      <li id="faq-contain-div" style="--tw-bg-hover-opacity: 0.1" class="faq-item hover-bg-primary rounded-lg cursor-pointer py-2 px-3 group transition-all">
        <div class="question flex justify-between items-center group-hover:text-[var(--main-color)] transition-all">
          <span class="text-sm">${markdownToHTML(faq.question)}</span>
          <i class="fas fa-chevron-right text-xs w-4 h-4 flex items-center justify-center text-[var(--main-color)] transition-all"></i>
        </div>
        <div class="answer mt-2 text-sm">
          ${markdownToHTML(faq.answer)}
        </div>
      </li>
      `;
  });
  ...
  form.addEventListener("submit", async function (event) {
    ...
    const response = await fetch(
      "https://chatbot-server-2668.vercel.app/api/get-bot-message",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
  ...
  searchInput.addEventListener("input", function (e) {
    const lowerCaseUserTyped = e.target.value.toLowerCase().split(" ");

    faqItems.forEach((item) => {
      const itemText =
        item.firstElementChild.firstElementChild.textContent.toLowerCase();
      const isVisible = lowerCaseUserTyped.some((word) =>
