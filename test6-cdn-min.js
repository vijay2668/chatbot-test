...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  const fetch_initial = await fetch("https://web-production-949d.up.railway.app/api/fetch-chatbot", {
    method: "POST",
    headers: {
  ...
  const handleFaqSubmit = async (query) => {
    ...
    const response = await fetch("https://web-production-949d.up.railway.app/api/get-bot-message", {
      method: "POST",
      headers: {
        "Content-Type": "application/json"
      },
  ...
  faqs.forEach(function (faq) {
    ...
    button.className =
      "text-sm cursor-pointer rounded-xl disabled:cursor-not-allowed disabled:opacity-60 text-white px-2 py-1 w-fit h-fit whitespace-nowrap";
  ...
  if (chatbotUI?.messages.length !== 0) {
    messages = chatbotUI?.messages.sort((a, b) => b?.createdAt - a?.createdAt);
